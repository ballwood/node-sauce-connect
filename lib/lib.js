...
function download(requestLib, url, filename, directory) {

  var localPath = path.join(directory, filename);
  console.log('Downloading to path:', localPath);
  ...
  return new Promise(function (resolve, reject) {
    ...
    requestLib(url)
      ...
      .on('progress', function (state) {
        console.log('Received ' + Math.floor(state.size.transferred / 1024) + 'K...');
      })
...
function extract(compressionLib, file) {
  ...
  return compressionLib(file, zipPath.dir).then(function (filesInZip) {
    ...
    console.log('Found sauce connect binary in compressed file:' + sauceConnectBinary.path);
    console.log('Extracting to: ' + outputLocation);
...
function moveToLib(filePath) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    console.log('Moving sauce connect binary to: ' + newLocation);
...
function setExecutePermissions(platform, filePath) {
