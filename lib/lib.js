...
function getDownloadFilename(arch) {
  return manifest[arch] && getFilenameFromUri(manifest[arch].download_url);
}

function getChecksum(arch) {
  return manifest[arch] && manifest[arch].sha1;
}
...
function download(logger, requestLib, uri, directory) {
...
function verifyFileChecksum(algorithm, checksum, localPath) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    stream.on('end', function () {
      var downloadedChecksum = hash.digest('hex') || '';

      if (checksum !== downloadedChecksum) {
        return reject(new Error('Downloaded checksum [' + downloadedChecksum + '] does not match [' + checksum + ']'));
      }
